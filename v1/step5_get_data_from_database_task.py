...
from utils.tool import random_color
...
class ConnectDdatabase(object):
    ...
    def connect_database(self) -> Engine:
        ...
        logger.info (random_color(f"{inspector} Successfully connected to the database"))
        return engine
    ...
    def write_to_database(self, data: pd.DataFrame) -> Any:
        try:
            ...
            logger.error (random_color(e))
            return
        finally:
            logger.info (random_color(f"Write successfully to {self.database} - {self.table}"))
    ...
    def save_local(self, data: pd.DataFrame) -> Any:
        ...
        try:
            ...
            logger.info (random_color(f"Successfully generate {self.save}"))
    
    def close_connect(self) -> Any:
        inspector = inspect(self.engine).default_schema_name
        self.engine.connect().close()
        logger.info (random_color(f"{inspector} The connection to the database '{self.database}' has been closed. Procedure"))
    
    def show(self) -> Any:
        get_tables = [tuple_table[0] for tuple_table in list(self.read_from_database("show tables")[1:])[0]]
        logger.info (
            random_color(f"{self.database} tables: {str(get_tables)}")
        )
